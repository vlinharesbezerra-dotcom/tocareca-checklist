...
SETORES = {
    ...
    'limpeza':  {'nome': 'Banheiro/Limpeza',  'emoji': '🧹'},
...
CHECKLISTS = {
    ...
    'salao': {
        'Externo':    ['Varrer o chão','Montar as mesas','Passar pano nas mesas','Repor os guardanapos'],
        'Interno':    ['Varrer o chão','Passar esfregão no chão','Limpar o armário','Limpar pote pimenta','Limpar pote azeite','Montar os pratos no armário','Repor guardanapos','Repor canudos'],
        'Palco':      ['Varrer o chão','Montar as mesas','Limpar as mesas','Repor guardanapos','Repor descartáveis'],
    },
    'bar': {
        'Abertura':   ['Ligar a chopeira','Conferir barril chopp','Conferir gás','Conferir frutas','Conferir as bebidas','Verificar gelo/pedir','Ligar luz da copa','Ligar impressora'],
        'Almoço':     ['Repor bebidas consumidas','Manter bancada organizada'],
        'Fechamento': ['Fechar garrafas e armazenar','Limpar e higienizar a copa','Desligar equipamentos'],
    },
    'caixa': {
    ...
    'limpeza': {
        'Abertura':   ['Tirar lixos do banheiro','Repor papel toalha','Repor papel higiênico','Repor sabonete','Limpar espelho','Esfregão no chão','Cheirinho'],
...
def checklist(setor):
    ...
    if setor == 'salao':
        if not sub:
            sub = 'Externo'
        itens = CHECKLISTS['salao'].get(sub, [])
        turnos_opcoes = ['Externo', 'Interno', 'Palco']
        turno = sub
    else:
        itens = CHECKLISTS.get(setor, {}).get(turno, [])
        turnos_opcoes = ['Abertura','Almoço','Tarde','Jantar','Fechamento']
    ...
    return render_template('checklist.html', setor=setor, info=SETORES[setor],
                           ...
                           turnos=turnos_opcoes, is_salao=(setor=='salao'))
